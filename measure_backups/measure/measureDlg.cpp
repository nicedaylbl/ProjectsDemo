...
void CmeasureDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_AXIS, m_StaticAxis);
}
...
BOOL CmeasureDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	((CButton*)GetDlgItem(IDC_CHECK_CORRENT))->SetCheck(1);
	((CButton*)GetDlgItem(IDC_CHECK_DWENBLE))->SetCheck(1);

	//初始化曲线表	
	pLeftAxis = myChartCtrl.CreateStandardAxis(CChartCtrl::LeftAxis);
	pLeftAxis->SetAutomatic(false);
	pLeftAxis->SetMinMax(0,500);
	//pAxis->EnableScrollBar(true);
	pLeftAxis->SetPanZoomEnabled(false);
	pBottomAxis = myChartCtrl.CreateStandardAxis(CChartCtrl::BottomAxis);
	pBottomAxis->SetAutomatic(false);
	pBottomAxis->EnableScrollBar(true);	
	pBottomAxis->SetMinMax(0,200);	

	TChartString strtitle = _T("功耗图表");
	myChartCtrl.GetTitle()->AddString(strtitle);
	TChartString strLeftAxis = _T("电流（mA）");
	...
	myChartCtrl.ShowMouseCursor(true);
	CMyCursorListener* pCursorListener = new CMyCursorListener;
	pCrossHair->RegisterListener(pCursorListener);
...
{
	//rate单位为ms
	int tempMinLeftAxis = 0; 
	int tempSample = rate >= 1000 ? (rate/1000) : rate;	
	
	if ( myCount == 1 )
	{
		if (ceil(myrealCurrent) < 500)
		{
			maxLeftAxis = 500;
		}
		else if (ceil(myrealCurrent) < 1000 )
		{
			maxLeftAxis = 1000;
		}
		else if (ceil(myrealCurrent) < 1500)
		{
			maxLeftAxis = 1500;
		}
		else 
		...
		tempMinLeftAxis = (maxLeftAxis - 500) > 0 ?  (maxLeftAxis - 500) : 0;
		pLeftAxis->SetMinMax(tempMinLeftAxis, maxLeftAxis);
	}

	if ( myCount <= MAXDWPONIT )
	{
		pBottomAxis->SetMinMax(0,MAXDWPONIT*tempSample);
	}
	else
	{
		...
		pBottomAxis->SetMinMax(startAxis,endAxis);
	}	
}
...
void CmeasureDlg::CurrentMeasures()
{	
...
void CmeasureDlg::OnBnClickedButtonStart()
{
	...
	if (!str.Compare(_T("开始")))
	{
		myCurrentData->clear();//
		//判断是否画图
		if ( BST_CHECKED == IsDlgButtonChecked( IDC_CHECK_DWENBLE ))
		{
			isDrawFlag = true;	
		}
		else
